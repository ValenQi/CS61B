...
class Game {
    ...
    /** Save the current state of the game to a sequence of states.*/
    void save() {
        _states.add(new Game(this));
    }
    ...
    /** Return the #J card from the top of foundation pile #K, where
     *  1 <= K <= NUM_SUITS, 0 <= J, or null if there is no such card.
     *  Throws IllegalArgumentException iff K is out of range. */
    Card getFoundation(int k, int j) {
        try {
            return foundation(k).get(j);
        } catch (IndexOutOfBoundsException excp) {
            throw err("no such fo foundation pile");
        }
    }

    /** Return the top card of foundation pile #K, where 1 <= K <= NUM_SUITS.
     *  Returns null if there is no such card.  Throws IllegalArgumentException
     *  iff K is out of range. */
    Card topFoundation(int k) {
        return getFoundation(k, 0);
    }
    ...
    /** Move the game back one move. By repeating it enough times, you can
     *  return to the initial state of a game immediately after the deal.
     *  Trying to undo the initial position has no effect.*/
    void undo() {
        if (!_states.isEmpty()) {
            this.copyFrom(_states.get(_states.size() - 1));
    ...
    /** A sequence of states of the game. */
    private final ArrayList<Game> _states = new ArrayList<>();
